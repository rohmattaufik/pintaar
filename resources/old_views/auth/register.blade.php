...
<form id="sign_in" role="form" method="POST" action="{{ route('register') }}">
    ...
    <div class="input-group">
        ...
        <div class="form-line {{ $errors->has('alamat') ? ' error' : '' }}">
            <input type="text" class="form-control" name="alamat" value="{{old('alamat')}}" placeholder="Address" autofocus>
        </div>
        @if ($errors->has('alamat'))
            <label id="name-error" class="error" for="name">{{ $errors->first('alamat') }}</label>
        @endif
    </div>
    <div class="input-group">
        <span class="input-group-addon">
